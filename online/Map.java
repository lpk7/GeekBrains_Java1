...
public class Map extends JPanel {
    ...
    private static int stateTurn = 0;

    private static final String MSG_WIN_HUMAN = "Победил игрок!";
    private static final String MSG_WIN_HUMAN_2 = "Победил второй\nигрок!";
    ...
    Map() {
        isInitialized = false;
        try {
            imgStart = ImageIO.read(Map.class.getResourceAsStream("Hello.jpg"));
            imgX = ImageIO.read(Map.class.getResourceAsStream("cross.png"));
            imgO = ImageIO.read(Map.class.getResourceAsStream("zero.png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void render(Graphics g) {
        ...
        if (!isInitialized) {
            g.drawImage(imgStart, 0, 0, width, height, null);
        } else {
            ...
            for (int y = 0; y < fieldSizeY; y++) {
                for (int x = 0; x < fieldSizeX; x++) {
                    ...
                    if (field[y][x] == DOT_HUMAN) {
                        g.drawImage(imgX, x * cellWidth, y * cellHeight, cellWidth, cellHeight, null);
                    } else if (field[y][x] == DOT_AI) {
                        g.drawImage(imgO, x * cellWidth, y * cellHeight, cellWidth, cellHeight, null);
    ...
    private void showMessageGameOver(Graphics g) {
        ...
        switch (stateGameOver) {
            ...
            case STATE_WIN_HUMAN_2:
                g.drawString(MSG_WIN_HUMAN_2, 3, getHeight() / 2);
                break;
            default:
    ...
    private void update(MouseEvent e) {
        if (!isInitialized) {
            try {
                imgStart = ImageIO.read(Map.class.getResourceAsStream("start.jpg"));
                repaint();
